...
do
    ...
    do
        ...
        ghc -w --make -O -threaded ${file%.*}.hs -o ${file%.*}.out -main-is $(basename ${file%.*}) -package-db $twd/../.cabal-sandbox/x86_64-linux-ghc-7.10.2-packages.conf.d  2> ${file%.*}.stderr 1> /dev/null # do not print ghc stdout, but redirect COMPILE ERRORS
...
do
    ...
    do
        ...
        echo "${i})Compiling ${file%.*} with ghc" ; \
        ghc -w --make -O -threaded ${file%.*}.hs -o ${file%.*}.out -main-is $(basename ${file%.*}) -package-db $twd/../.cabal-sandbox/x86_64-linux-ghc-7.10.2-packages.conf.d ; }
        echo "${i})Running ${file%.*}"
        ${file%.*}.out 1> ${file%.*}.stdout 2> ${file%.*}.stderr  # do not print exe stdout, but redirect RUNTIME ERRORS
        if [ $? -eq 0 ]; then
